...
#'
#' @description Simple front-end for pulling in data
#'
...
#' @param multco logical. Return dataset for analyses in Multnomah County only. The only option
...
#'
#' \dontrun{
#'  get_data_hys(cols = NULL, year = c(2016, 2018), weight_variable = 'wt_sex_grade_mc')
#' }
get_data_hys <- function(cols = NULL, year = c(2021), weight_variable = 'wt_sex_grade_mc', multco = TRUE, version = 'best', ar = TRUE){

  colname <- phi_geo_mc <- weight1 <- psu <- phi_year <- NULL
  ...
  #subset by year
  yvar = year
  dat = dat[phi_year %in% yvar, ]

  dat = dat[get(weight_variable)>0]
  svy <- dtsurvey::dtsurvey(dat, psu = 'psu', strata = 'phi_year', weight = weight_variable, nest = T)
...
#'
#' @param multco Logical. Return dataset for analyses where mother's residence is in Multnomah County only.
#'
#' Default = T
#'
...
get_data_birth <- function(cols = NA,
  ...
  # validate arguments other than mykey ----
    ...
    if((length(multco) == 1 && is.na(multco)) | !is.logical(multco)){stop('\n\U0001f6d1 `multco` must be a logical (TRUE | FALSE, or equivalently, T | F).')}
  ...
  # get list of all colnames from SQL ----
    ...
    birth.years <- unique(DBI::dbGetQuery(con, glue::glue_sql("SELECT DISTINCT phi_year FROM {mysqltable}"))$phi_year)
  ...
  # pull columns and years from SQL ----
    ...
    query.string <- paste0("SELECT ", paste0(cols, collapse = ', '), " FROM ", mysqltable, " WHERE phi_year IN (", paste0(validyears, collapse = ', '), ")")

    if(multco == T){query.string <- paste0(query.string, " AND phi_geo_mc = 'Multnomah County'")}
...
#' Default = most recent year only
#'
#' @param multco logical. Return dataset for analyses where county of decedent's residence is Multnomah County.
#'
#' Default = T
#'
#' @param version Character vector of length 1. Either 'final' or 'stage'.
...
#' Default = 'final'
#'
#' @param topcode logical. Do you want to top code phi_age at 100 to match population data?
#'
#' Default = T
#'
#' @param mykey Character vector of length 1. Identifies
...
get_data_death <- function(cols = NA,
  ...
  phi_age <- date_of_birth <- date_of_death <- age_years <- phi_race_eth7 <- NULL
  phi_race_6 <- phi_race_eth8 <- phi_race_7 <- geo_id_code <- phi_sex <-  NULL
  age6 <- phi_geo_mc <- phi_geo_wastate <- phi_race_hisp <- geo_id <- geo_type <- NULL
  race3 <- race3_hispanic <- race4 <- wastate <- bigcities <- phi_geo_bigcities <- NULL
  hra20_name <- phi_geo_hra2020_long <- phi_geo_region <- phi_geo_reg2020_name <- NULL
  ...
  # Get list of all colnames from SQL ----
      ...
      death.years <- sort(unique(DBI::dbGetQuery(con, glue::glue_sql("SELECT DISTINCT phi_year FROM {mysqltable}"))$phi_year))

  # Identify columns and years to pull from SQL ----
      cols <- tolower(cols)
      if(!all(is.na(cols))){
        # for custom PHI/CHNA vars
        original.cols <- copy(cols)
        var.2.calc <- c()
        if('age6' %in% cols){cols <- c(cols, 'phi_age'); var.2.calc=c(var.2.calc, 'age6')}
        if('bigcities' %in% cols){cols <- c(cols, 'phi_geo_bigcities'); var.2.calc=c(var.2.calc, 'bigcities')}
        if('phi_geo_region' %in% cols){cols <- c(cols, 'phi_geo_reg2020_name'); var.2.calc=c(var.2.calc, 'phi_geo_region')}
        if('hra20_name' %in% cols){cols <- c(cols, 'phi_geo_hra2020_long'); var.2.calc=c(var.2.calc, 'hra20_name')}
        if('race3' %in% cols){cols <- c(cols, 'phi_race_6', 'phi_race_hisp'); var.2.calc=c(var.2.calc, 'race3')}
        if('race4' %in% cols){cols <- c(cols, 'phi_race_eth7'); var.2.calc=c(var.2.calc, 'race4')}
        if('wastate' %in% cols){cols <- c(cols, 'phi_geo_wastate'); var.2.calc=c(var.2.calc, 'wastate')}
  ...
  # Pull columns and years from SQL ----
      ...
      if(multco == T){query.string <- paste0(query.string, " AND phi_geo_mc = 1")}
  ...
  # Top code age (if wanted) ----
      if( 'phi_age' %in% cols | 'phi_age' %in% names(dat) ){
        dat[phi_age < 0, phi_age := NA] # cannot have a negative age (due to 9999 as year of birth)
        if(topcode == T){
          dat[phi_age > 100, phi_age := 100] # top code to 100 to match population data
        }
      }
  ...
  # Label race_ethnicity data ----
      if( 'phi_race_eth7' %in% cols | 'phi_race_eth7' %in% names(dat) ){
        dat[, phi_race_eth7 := factor(phi_race_eth7,
                                    levels = c(2, 1, 5, 7, 8, 6, 3),
                                    labels = c("Black", "White", "Multiple", "Asian", "NHPI", "Hispanic", "AIAN"))]
      }
      if( 'phi_race_eth8' %in% cols | 'phi_race_eth8' %in% names(dat) ){
        dat[, phi_race_eth8 := factor(phi_race_eth8,
                                      levels = c(2, 1, 5, 7, 8, 6, 3, 9),
                                      labels = c("Black", "White", "Multiple", "Asian", "NHPI", "Hispanic", "AIAN", "Oth/unk"))]
      }
      if( 'phi_race_6' %in% cols | 'phi_race_6' %in% names(dat) ){
        dat[, phi_race_6 := factor(phi_race_6,
                                      levels = c(3, 6, 4, 2, 1, 5),
                                      labels = c("Black", "White", "Multiple", "Asian", "AIAN", "NHPI"))]
      }
      if( 'phi_race_7' %in% cols | 'phi_race_7' %in% names(dat) ){
        dat[, phi_race_7 := factor(phi_race_7,
                                   levels = c(3, 6, 4, 2, 1, 5, 9),
                                   labels = c("Black", "White", "Multiple", "Asian", "AIAN", "NHPI", "Oth/unk"))]
      }

  # Label gender ----
      if("phi_sex" %in% names(dat)){
        dat[, phi_sex := factor(phi_sex, levels = 0:1, labels = c("Female", "Male"))]
      }

  # Create custom PHI vars if requested ----
      if('bigcities' %in% original.cols || (length(original.cols) == 1 && is.na(original.cols))){
        dat[, bigcities := phi_geo_bigcities]
      }
      if('phi_geo_mc' %in% original.cols || (length(original.cols) == 1 && is.na(original.cols))){
        dat[, phi_geo_mc := as.character(phi_geo_mc)]
        dat[, phi_geo_mc := fcase(phi_geo_mc=='TRUE', 'Multnomah County',
                                  default = NA_character_)]
      }
      if('hra20_name' %in% original.cols || (length(original.cols) == 1 && is.na(original.cols))){
        dat[, hra20_name := phi_geo_hra2020_long]
      }
      if('phi_geo_region' %in% original.cols || (length(original.cols) == 1 && is.na(original.cols))){
        dat[, phi_geo_region := phi_geo_reg2020_name]
      }
      if('wastate' %in% original.cols || (length(original.cols) == 1 && is.na(original.cols))){
        dat[phi_geo_wastate == TRUE, wastate := 'Washington State']
      }
      if('age6' %in% original.cols || (length(original.cols) == 1 && is.na(original.cols))){
        dat[, age6 := fcase(phi_age %in% 0:17, '<18',
                            phi_age %in% 18:24, '18-24',
                            phi_age %in% 25:44, '25-44',
                            phi_age %in% 45:64, '45-64',
                            phi_age %in% 65:74, '65-74',
                            phi_age >= 75, '75+',
                            default = NA_character_)]
      }
      if('race4' %in% original.cols || (length(original.cols) == 1 && is.na(original.cols))){
        dat[, race4 := phi_race_eth7]
      }
      if('race3' %in% original.cols | (length(original.cols) == 1 && is.na(original.cols))){
        dat[, race3 := phi_race_6]
        dat[phi_race_hisp == 1, race3_hispanic := 'Hispanic']
      }
...
#'
#' Default = most recent year only
#' @param multco logical OR 'zip'.
#'
#' When `multco = T`,  returns dataset where the patient county is Multnomah County -- based on country code.
#'
#' When `multco = 'zip'`, returns a dataset where the patient county is Multnomah County -- based on zip code.
#'
#' Default = T
#'
...
#'
#' \dontrun{
#'  get_data_chars(cols = c('seq_no', 'phi_age'),
#'                 year = c(2020),
#'                 multco = T,
#'                 wastate = T,
#'                 version = 'final',
...
get_data_chars <- function(cols = NA,
  ...
  phi_age <- date_of_birth <- date_of_chars <- age_years <- phi_race_eth7 <- NULL
  phi_race_6 <- phi_race_eth8 <- phi_race_7 <- geo_id_code <- phi_geo_wastate <- NULL
  phi_sex <- phi_geo_mc <- phi_race_aic_hisp <- yage4 <- age <- age6 <- NULL
  ...
  if((length(multco) == 1 && is.na(multco)) | !(is.logical(multco) | multco == 'zip') ){stop('\n\U0001f6d1 `multco` must be a logical (TRUE | FALSE, or equivalently, T | F).')}
  ...
  # Get list of all colnames from SQL ----
      ...
      chars.years <- sort(unique(DBI::dbGetQuery(con, glue::glue_sql("SELECT DISTINCT phi_year FROM {mysqltable}"))$phi_year))
  ...
  # Identify columns and years to pull from SQL ----
      ...
      if(!all(is.na(cols))){
        # for custom PHI/CHNA vars
          ...
          if('race3' %in% cols){cols <- c(cols, 'phi_race_6', 'phi_race_aic_hisp'); var.2.calc=c(var.2.calc, 'race3')}
          if('race4' %in% cols){cols <- c(cols, 'phi_race_eth7'); var.2.calc=c(var.2.calc, 'race4')}
          cols <-  unique(setdiff(cols, var.2.calc))
        # generic vars
  ...
  # Pull columns and years from SQL ----
      ...
      if(wastate == T){query.string <- glue:: glue_sql (query.string, " AND phi_geo_wastate = 1")}
      if(multco == T){query.string <- glue:: glue_sql (query.string, " AND phi_geo_mc = 1")}
      if(multco == 'zip'){query.string <- glue:: glue_sql (query.string, " AND phi_geo_mczip = 1")}
  ...
  # Label gender ----
      if( 'phi_sex' %in% cols | 'phi_sex' %in% names(dat) ){
        dat[, phi_sex := factor(phi_sex, levels = 0:1, labels = c("Female", "Male"))]
      }

  # Create custom PHI vars if requested ----
    if('phi_geo_mc' %in% names(dat)){
      dat[, phi_geo_mc := as.character(phi_geo_mc)]
      dat[, phi_geo_mc := fcase(phi_geo_mc=='TRUE', 'Multnomah County',
                                default = NA_character_)]
    }
    ...
    if('race3' %in% original.cols | (length(original.cols) == 1 && is.na(original.cols))){
      dat[, race3 := phi_race_6]
      dat[phi_race_aic_hisp == T, race3_hispanic := 'Hispanic']
    }
