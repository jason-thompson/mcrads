...
#'
#' @source
#' \code{mcrads.data::icd10_death_injury_matrix}
#'
#'# *** revise
#' @references
...
#' print(blah)
#'
#' @import data.table mcrads.data
#'
death_injury_matrix<- function(){
  # Global variables used by data.table declared as NULL here to play nice with devtools::check() ----
  death_injury_matrix_list <- mechanism <- intent <-  NULL

  death_injury_matrix_list <- unique(data.table::copy(mcrads.data::icd10_death_injury_matrix)[, list(mechanism, intent)])
...
#' \code{unique(mcrads.data::icd10_death_injury_matrix$mechanism)} in your
#' R console.
#'
#' **NOTE**
...
#'
#' @param multco a logical vector of length one. It specifies whether you want to
#' limit the analysis to Multnomah County. Note that this only works with data
#' imported from the \code{get_data_death()} function.
#'
#' The default is \code{multco = TRUE}.
#'
...
#' results. For example, if you specified \code{group_by = c('phi_sex',
#' 'phi_race_6')}, the results would be stratified by each combination of sex
#' and race.
#'
...
#' set.seed(98104)
#' injurydata4[, ageofdeath := mcrads::round2(rnorm(1, mean = 70, sd = 5 ), 0),
...
death_injury_matrix_count <- function(ph.data = NULL,
  ...
  # Check arguments ----
    ...
    if(length(grep("\\.|-", ph.data[[icdcol]], value = T) >0 )){
      warning(paste0("
          ...
          This is necessary because causeids in mcrads.data::icd10_death_injury_matrix contains no hyphens or periods."
      ))
  ...
  if("*" %in% intent){x_intent = unique(mcrads.data::icd10_death_injury_matrix$intent)}
  if("none" %in% intent){x_intent = unique(mcrads.data::icd10_death_injury_matrix$intent)} # "Any intent" will be sum of all of intents
  if(length(intersect(c("*", "none"), intent)) == 0){
    x_intent = c()
    for(i in intent){
      x_intent <- unique(c(x_intent, grep(i, unique(mcrads.data::icd10_death_injury_matrix$intent), value = TRUE, ignore.case = TRUE)))
    }
  }
  ...
  if("*" %in% mechanism){x_mechanism = unique(mcrads.data::icd10_death_injury_matrix$mechanism)}
  if("none" %in% mechanism){x_mechanism = "All injury"}
  if(length(intersect(c("*", "none"), mechanism)) == 0){
    x_mechanism = c()
    for(i in mechanism){
      x_mechanism <- unique(c(x_mechanism, grep(i, unique(mcrads.data::icd10_death_injury_matrix$mechanism), value = TRUE, ignore.case = TRUE)))
    }
  }
  ...
  # Count deaths for each intent_x_mechanism of interest ----
    ...
    # get reference table from mcrads.data
    x_reftable <- copy(mcrads.data::icd10_death_injury_matrix)[, orig.coding := NULL]
...
#'
#' @details
#' See \code{mcrads::death_113()} & \code{mcrads::death_130()} for a complete list
...
#' function because it needs the logical variable \code{chi_geo_mc}.
#'
...
#' COD (\code{mcrads::death_113()}) or NCHS 130 Selected Causes of Infant Death
#' (\code{mcrads::death_130()}).
#'
#' @references
...
death_xxx_count <- function(ph.data = NULL,
  ...
  # Global variables used by data.table declared as NULL here to play nice with devtools::check() ----
    ...
    chi_geo_mc <- underlying_cod_code <- icd10 <- NULL
  ...
  # Check arguments ----
            ...
            warning(
              paste0(
                ...
                ") that contains a hyphen (-), period (.), space or some other non alpha-numeric character.
                  These characters will be deleted, e.g., A85.2 will become A852.
                  This is necessary because causeids the mcrads.data::icd_nchs###causes tables contain no hyphens or periods."
              )
            )
            ph.data[, paste0(icdcol) := gsub("[[:space:].]+", "", gsub("([^A-Za-z0-9 ])+", "", x = get(icdcol)))]
      ...
      # check that kingco is a logical ----
          ...
          if (isTRUE(kingco)) {
            ph.data <- ph.data[chi_geo_mc == 'King County']
          }
      ...
      if (isFALSE(is.null(cause))) {
        ...
        for (i in cause) {
          if(nchsnum == 113){
            x_cause <-
              unique(c(x_cause, grep(i,
                                     unique(mcrads.data::icd_nchs113causes[]$cause.of.death),
                                     value = TRUE,
                                     ignore.case = TRUE
            )))
          }
          if(nchsnum == 130){
            x_cause <-
              unique(c(x_cause, grep(i,
                                     unique(mcrads.data::icd_nchs130causes[]$cause.of.death),
                                     value = TRUE,
                                     ignore.case = TRUE
            )))
          }
      ...
      if (isFALSE(is.null(cause)) & length(x_cause) == 0) {
        stop(
          paste0(
            ...
            To view all available cause of death, type the following in your console: unique(mcrads.data::icd_nchs", nchsnum, "causes$cause.of.death)"
          )
        )
      }
  ...
  # Count deaths for each cause ----
    # prep causes of death reference table ----
      # get reference table from mcrads.data
      if(nchsnum == 113){x_reftable <- copy(mcrads.data::icd_nchs113causes)}
      if(nchsnum == 130){x_reftable <- copy(mcrads.data::icd_nchs130causes)}
...
#'
#' @source
#' \code{mcrads.data::icd_nchs113causes_raw}
#'
#' @references
#' \url{https://www.cdc.gov/nchs/data/dvs/Part9InstructionManual2020-508.pdf} &
#' \url{https://secureaccess.wa.gov/doh/chat/Content/FilesForDownload/CodeSetDefinitions/NCHS113CausesOfDeath.pdf}
...
death_113<- function(){
  ...
  deaths113_causeid_list <- copy(mcrads.data::icd_nchs113causes_raw)
  deaths113_causeid_list <- deaths113_causeid_list[, .SD, .SDcols = c("causeid", "cause.of.death")] # from mcrads.data
...
#' COVID-19 deaths (since they do not have their own NCHS category).
#'
#' See \code{mcrads::death_113()} for a complete list of available causesid
#' and cause values.
#'
#' @details
#' There are actually 114 rows, with causeid 114 being the official
...
#'
#' @note
#' Calls upon \code{mcrads::death_xxx_count}.
#'
#' @export
#'
...
#' set.seed(98104)
#' deathdata2[, ageofdeath := mcrads::round2(rnorm(1, mean = 70, sd = 5 ), 0),
...
#' hierarchy), check out \code{?mcrads.data::icd_nchs130causes_raw} &
#' \code{?mcrads.data::icd_nchs130causes}.
#'
#' @note
#' This function does not take any arguments
#'
#' @source
#' \code{mcrads.data::icd_nchs130causes_raw}
#'
#' @references
...
death_130<- function(){
  ...
  deaths130_causeid_list <- copy(mcrads.data::icd_nchs130causes_raw)
  deaths130_causeid_list <- deaths130_causeid_list[, .SD, .SDcols = c("causeid", "cause.of.death")] # from mcrads.data
...
#' COVID-19 deaths (since they do not have their own NCHS category).
#'
#' See \code{mcrads::death_130()} for a complete list of available causesid
#' and cause values.
#'
#' @details
...
# death_other() ----
#' View "Other" Causes of Death available in mcrads
#'
#' @description
#' Function to view "Other" Causes of Death that are availbe in mcrads (via
#' \href{https://github.com/PHSmc-APDE/mcrads.data}{mcrads.data}). These are causes
...
#' column from \code{mcrads.data::icd_other_causes_of_death}.
#'
#' @note
#' This function does not take any arguments
#'
#' @source
#' \code{mcrads.data::icd_other_causes_of_death}
#'
#' @references
#' \code{?mcrads.data::icd_other_causes_of_death}
#'
...
death_other<- function(){
  ...
  death_other_list <- data.table::copy(mcrads.data::icd_other_causes_of_death)
  death_other_list <- unique(death_other_list$cause.of.death) # from mcrads.data
...
death_other_count <- function(ph.data = NULL,
  ...
  x_reftable <- x_combo <- x_covid <- x_cause <- x_all <- x_ypll <- NULL
  chi_geo_mc <- underlying_cod_code <- NULL
  date_of_death <- date_of_birth <- calculated.age <- orig.coding <- NULL
  ...
  # Check arguments ----
    ...
    # icdcol ----
      ...
      if(length(grep("\\.|-", ph.data[[icdcol]], value = T) >0 )){
        warning(paste0("
              There is at least one row where `icdcol` (", icdcol, ") contains a hyphen (-), period (.), space or some other non alpha-numeric character.
              These characters will be deleted, e.g., A85.2 will become A852.
              This is necessary because causeids in mcrads.data::icd_other_causes_of_death contains no hyphens or periods."
        ))
        ph.data[, paste0(icdcol) := gsub("[[:space:].]+", "", gsub("([^A-Za-z0-9 ])+", "", x = get(icdcol)))]
    ...
    if(isFALSE(is.null(ypll_age)) & is.null(death_age_col) & length(intersect(c("date_of_birth", "date_of_death"), colnames(ph.data))) == 2){
      if(isFALSE(class(ph.data[['date_of_birth']]) == "Date") | isFALSE(class(ph.data[['date_of_birth']]) == "Date")){
        ...
        ph.data[date_of_death >= date_of_birth, calculated.age := mcrads::calc_age(date_of_birth, date_of_death)]
  ...
  if(isFALSE(is.null(cause))){
    ...
    for(i in cause){
      x_cause <- unique(c(x_cause,
                          grep(i, unique(mcrads.data::icd_other_causes_of_death[]$cause.of.death),
                               value = TRUE,
                               ignore.case = TRUE)))
    }
  }
  ...
  # Count deaths for each cause of death ----
    # prep cause of death reference table ----
      # get reference table from mcrads.data
      x_reftable <- copy(mcrads.data::icd_other_causes_of_death)[, orig.coding := NULL]
...
life_table <- function(ph.data = NULL,
  ...
  # rounding
  ph.data[, c("lx", "dx", "Lx", "Tx", mydeaths) := lapply(.SD, mcrads::round2, 0), .SDcols = c("lx", "dx", "Lx", "Tx", mydeaths)]
  ph.data[, c("qx", "mx") := lapply(.SD, mcrads::round2, 5), .SDcols = c("qx", "mx")]
  ph.data[, c("ex_se") := lapply(.SD, mcrads::round2, 5), .SDcols = c("ex_se")]
  ph.data[, c("ax", "ex", "ex_lower", "ex_upper") := lapply(.SD, mcrads::round2, 2), .SDcols = c("ax", "ex", "ex_lower", "ex_upper")]
...
life_table_prep <- function(DTx = NULL,
  ...
  DTx[, death_age := mcrads::calc_age(dob, dod)]
