...
# Check death_113_count ----
  ...
  test_that("Check for proper triggering of errors ...", {
    expect_error(death_113_count(ph.data = "d113data", multco = FALSE)) # name of data.table must be unquoted
    expect_warning(death_113_count(ph.data = d113data, icdcol = "cod.icd10", multco = FALSE)) # warning because of period in A85.2
    expect_error(suppressWarnings(death_113_count(ph.data = d113data, multco = TRUE))) # Should error because there is no KC data here
    expect_error(death_113_count(ph.data = d113data, causeids = seq(1, 115, 1), multco = FALSE)) # Should error because highest causeid is 114
    expect_error(death_113_count(ph.data = d113data, causeids = seq(0, 114, 1), multco = FALSE)) # Should error because lowest causeid is 0
    expect_error(death_113_count(ph.data = d113data, causeids = c(2, 5, "7", 13), multco = FALSE)) # Should error because of a non-numeric causeid
    expect_error(death_113_count(ph.data = d113data, causeids = c(2, 5, 7.3, 13), multco = FALSE)) # Should error because of a non-integer causeid
    expect_error(death_113_count(ph.data = d113data, icdcol = cod.icd10, multco = FALSE)) # Should error because icdcol should be quoted
    expect_error(death_113_count(ph.data = d113data, icdcol = "cod.icd10x", multco = FALSE)) # Should error because icdcol does not exist
    expect_error(suppressWarnings(death_113_count(ph.data = d113data2, icdcol = "cod.icd10", multco = FALSE, ypll_age = "65"))) # Should error because ypll_age is character
    expect_error(suppressWarnings(death_113_count(ph.data = d113data2, icdcol = "cod.icd10", multco = FALSE, ypll_age = 65))) # Should error bc need to specify dob/dod or death_age_col
    expect_error(suppressWarnings(death_113_count(ph.data = d113data2, icdcol = "cod.icd10", multco = FALSE, ypll_age = 65, death_age_col = ageofdeath))) # Should error ageofdeath not quoted
    expect_error(suppressWarnings(death_113_count(ph.data = d113data2, icdcol = 'cod.icd10', multco = F, group_by = c('stratum')))) # stratum does not exist, should be `strata`
  })
  ...
  test_that("Death counts by cause are accurate ...", {
    ...
    expect_equal(suppressWarnings(sum(death_113_count(ph.data = d113data2, icdcol = 'cod.icd10', multco = F, group_by = c('strata'))[cause.of.death == 'All causes']$deaths)),
  ...
  test_that("Structure of output table is as expected ...", {
    ...
    expect_equal(nrow(suppressWarnings(death_113_count(ph.data = d113data2, icdcol = "cod.icd10", multco = FALSE, ypll_age = 65, death_age_col = "ageofdeath"))),
                 10) # eight causeids should be present, PLUS COVID, PLUS 'All causes'
    expect_equal(sort(names(suppressWarnings(death_113_count(ph.data = d113data2, icdcol = "cod.icd10", multco = FALSE, ypll_age = 85, death_age_col = "ageofdeath")))),
                 c("cause.of.death", "causeid", "deaths", "ypll_85"))
    expect_equal(names(suppressWarnings(death_113_count(ph.data = d113data2, icdcol = 'cod.icd10', multco = F, group_by = c('strata')))[]),
                 c('cause.of.death', 'causeid', 'deaths', 'strata'))
    expect_equal(sort(unique(suppressWarnings(death_113_count(ph.data = d113data2, icdcol = 'cod.icd10', multco = F, group_by = c('strata')))[]$strata)),
...
# Check death_130_count ----
  ...
  test_that("Check for proper triggering of errors ...", {
    expect_error(death_130_count(ph.data = "d130data", multco = FALSE)) # name of data.table must be unquoted
    expect_warning(death_130_count(ph.data = d130data, icdcol = "cod.icd10", multco = FALSE)) # warning because of period in A85.2
    expect_error(suppressWarnings(death_130_count(ph.data = d130data, multco = TRUE))) # Should error because there is no KC data here
    expect_error(death_130_count(ph.data = d130data, causeids = seq(1, 115, 1), multco = FALSE)) # Should error because highest causeid is 114
    expect_error(death_130_count(ph.data = d130data, causeids = seq(0, 114, 1), multco = FALSE)) # Should error because lowest causeid is 0
    expect_error(death_130_count(ph.data = d130data, causeids = c(2, 5, "7", 13), multco = FALSE)) # Should error because of a non-numeric causeid
    expect_error(death_130_count(ph.data = d130data, causeids = c(2, 5, 7.3, 13), multco = FALSE)) # Should error because of a non-integer causeid
    expect_error(death_130_count(ph.data = d130data, icdcol = cod.icd10, multco = FALSE)) # Should error because icdcol should be quoted
    expect_error(death_130_count(ph.data = d130data, icdcol = "cod.icd10x", multco = FALSE)) # Should error because icdcol does not exist
    expect_error(suppressWarnings(death_130_count(ph.data = d130data2, icdcol = "cod.icd10", multco = FALSE, ypll_age = "65"))) # Should error because ypll_age is character
    expect_error(suppressWarnings(death_130_count(ph.data = d130data2, icdcol = "cod.icd10", multco = FALSE, ypll_age = 65))) # Should error bc need to specify dob/dod or death_age_col
    expect_error(suppressWarnings(death_130_count(ph.data = d130data2, icdcol = "cod.icd10", multco = FALSE, ypll_age = 65, death_age_col = ageofdeath))) # Should error ageofdeath not quoted
...
# Check death_injury_matrix_count ----
  ...
  test_that("Check for proper triggering of errors ...", {
    expect_error(death_injury_matrix_count(ph.data = "injurydata", multco = FALSE)) # name of data.table must be unquoted
    expect_error(death_injury_matrix_count(ph.data = injurydatum, multco = FALSE)) # warning because data.table doesn't exist
    expect_error(suppressWarnings(death_injury_matrix_count(ph.data = injurydata, multco = TRUE))) # Should error because there is no KC data here
    expect_error(death_injury_matrix_count(ph.data = injurydata, intent = "z", multco = FALSE)) # Should error because none of the intents have 'z'
    expect_error(death_injury_matrix_count(ph.data = injurydata, mechanism = "z", multco = FALSE)) # Should error because none of the mechanisms have 'z'
    expect_error(death_injury_matrix_count(ph.data = injurydata, intent = 100, multco = FALSE)) # Should error because intent must be a character
    expect_error(death_injury_matrix_count(ph.data = injurydata, mechanism = 100, multco = FALSE)) # Should error because mechanism must be a character
    expect_error(death_injury_matrix_count(ph.data = injurydata, icdcol = cod.icd10, multco = FALSE)) # Should error because icdcol should be quoted
    expect_error(death_injury_matrix_count(ph.data = injurydata, icdcol = "cod.icd10x", multco = FALSE)) # Should error because icdcol does not exist
    expect_error(death_injury_matrix_count(ph.data = injurydata, icdcol = "cod.icd10", multco = FALSE, ypll_age = 65)) # Should error because data lacks dob/dod
    expect_error(death_injury_matrix_count(ph.data = injurydata4, icdcol = "cod.icd10", multco = FALSE, ypll_age = 65)) # Should error because didn't specify age
    expect_error(death_injury_matrix_count(ph.data = injurydata5, icdcol = "cod.icd10", multco = FALSE, ypll_age = "65")) # Should error because ypll_age is character
    expect_error(death_injury_matrix_count(ph.data = injurydata5, icdcol = "cod.icd10", multco = FALSE, ypll_age = 65.1)) # Should error because ypll_age is not integer
    expect_error(death_injury_matrix_count(ph.data = injurydata5, icdcol = "cod.icd10", multco = FALSE, group_by = 'stratum')) # Should error because colunmn is `strata`, not stratum
  })
  ...
  test_that("Death counts are accurate ...", {
    ...
    expect_equal(sum(death_injury_matrix_count(ph.data = injurydata5, icdcol = "cod.icd10", multco = FALSE, group_by = 'strata')[]$deaths),
                 sum(death_injury_matrix_count(ph.data = injurydata5, icdcol = "cod.icd10", multco = FALSE)[]$deaths)) # group_by should not impact total
  })
  ...
  test_that("Structure of output table is as expected ...", {
    ...
    expect_equal(sort(names(death_injury_matrix_count(ph.data = injurydata5, icdcol = "cod.icd10", multco = FALSE, group_by = 'strata'))),
                 sort(c('mechanism', 'intent', 'deaths', 'strata')))
    expect_equal(sort(unique(death_injury_matrix_count(ph.data = injurydata5, icdcol = "cod.icd10", multco = FALSE, group_by = 'strata')[]$strata)),
